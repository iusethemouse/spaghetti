...
import {
  state, COLORS, boot, refresh, subscribe, emit, report, card, tag, save, cancelRequests,
...
function openConnection() {
  ...
  menu.hidden = true;
  tokenInput.value = '';
  organizationInput.value = api.org;
  connectionError.hidden = true;
  disconnectButton.hidden = !api.connected;
  connection.showModal();
  tokenInput.focus();
}

connectionButton.addEventListener('click', openConnection);
document.getElementById('connection-close').addEventListener('click', () => connection.close());
connection.addEventListener('close', () => { tokenInput.value = ''; });
document.getElementById('token-form').addEventListener('submit', async (event) => {
  ...
  try {
    await save();
    await api.connect(tokenInput.value, organizationInput.value);
    cancelRequests();
    tokenInput.value = '';
    connection.close();
    await boot();
...
function pickRepo(index = state.board.repos.length) {
  if (api.hosted && !api.connected) return openConnection();
  picker.open({
    load: async (query) => {
...
install();
boot().then(() => {
  if (api.hosted && !api.connected) openConnection();
}).catch(report);
