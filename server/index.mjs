...
async function api(req, res, url) {
  ...
  if (route === 'viewer') {
    return send(res, 200, { viewer: await cached('viewer', viewer) });
  }
...
export async function start(assets = null) {
  ...
  if (!health.ok) throw new Error(`GitHub CLI is not ready: ${health.error}. Install gh and run gh auth login.`);
